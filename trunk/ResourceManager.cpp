...
bool ResourceManager::Init(NiStream* stream, NiRenderer* renderer)
{
	...
	if (!bSuccess) return false;
	bSuccess = LoadNif(stream, string("models/beeFinal.nif"), RES_MODEL_HONEYBEE);
	if (!bSuccess) return false;
	...
	if (!bSuccess) return false;
	bSuccess = LoadNif(stream, string("models/dragonfly_physx3.nif"), RES_MODEL_DRAGONFLY);
	if (!bSuccess) return false;
	bSuccess = LoadNif(stream, string("models/dragonfly_physx3.nif"), RES_MODEL_BOSS);
	if (!bSuccess) return false;
	bSuccess = LoadNif(stream, string("models/base1.nif"), RES_MODEL_BASE);
	if (!bSuccess) return false;
